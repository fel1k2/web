...
export const Asteroids = () => {
    ...
    useEffect(() => {
        try {
            const result = fetch(
                'https://api.nasa.gov/neo/rest/v1/feed?api_key=H94YRDb6SiGB53kYh5lC4N68Hj4gu9UcsrBhdOM5'
            )
                .then((res) => {
                    return res.json()
                })
            ...
            <div>
                <div>
                    <div>
                        ...
                        />
                        <label htmlFor={'onlyDanger'}>
                            Показывать только опасные
                        </label>
                    </div>

                    <div className={styles.distances}>
                        <label>Расстояние</label>
                        <Link onClick={() => setonlyhdistanceMode(true)} to="">
                            в километрах
                        </Link>
                        <Link onClick={() => setonlyhdistanceMode(false)} to="">
